...
const ConversationItem = ({active, time, name, venom,message,setter,item}) => {
    console.log(item)
    const _class = active ? 'bg-gray-200' : 'bg-white';
    return (
        <div>
            <div className={'conversation-item p-1 dark:bg-gray-700 hover:bg-gray-200 m-1 rounded-md ' + _class}  onClick={()=>{
                setter(venom)
            }}>
